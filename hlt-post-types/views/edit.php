...
if(isset($_POST['edit_save'])){
    ...
    if(!empty($field_name) && !empty($type) && !empty($status)  ){
        $wpdb->update( $hlt_pt_fields_table, 
        	array( 
        		'name' => $field_name, 
        		'type' => $type, 
                'options' => $options, 
        ...
        echo hlt_pt_display_msg('Field Record has been updated successfully',1);
    }
    else{
        //error
        echo hlt_pt_display_msg('All fields are required. Field Record was not added.',2);
    }
    
}

$field = $wpdb->get_row("SELECT * FROM $hlt_pt_fields_table WHERE id = $field_id");


?>

<div class="wrap">
<h2><?php echo $hlt_pt->name; ?>&nbsp; &gt;&gt; &nbsp;Update Field Record <a href="<?php echo admin_url('admin.php?page=hlt-pt-post-types&hlt_pg=field-settings&id='.$p_type_id);?>" class="add-new-h2">Back</a></h2>
