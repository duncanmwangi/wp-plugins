<?php
    ...
    $emails_not_to_award_sql_str = "'".implode("','",$emails_not_to_award)."'";
    ...
    $where = " WHERE COALESCE((COALESCE(b.order_amount,0) - COALESCE(c.rebate_amount,0)),0) >= $this->rebate_amount AND email NOT IN ($emails_not_to_award_sql_str) ";
    ...
    $allrowsxx = $this->db->get_results( $sql.$where );
    if(isset($_GET['action']) && $_GET['action']=='autoapprove')
        $dnc_rebates = $this->auto_issue_rebates($allrowsxx);
   //echo $sql.$where.$limit;
    ...
    <div class="wrap">
    <h2>Pending Rebates
    <a href="<?=admin_url('admin.php?page=drp-rebates&action=autoapprove')?>" class="page-title-action">Auto Approve All Pending Rebates</a>
    </h2>
    <form id="posts-filter" action="" method="get">
        <input type="hidden" name="page" value="drp-rebates" />
            ...
            if($allrows){
                ...
                foreach($allrows as $row){
                    ...
                    <tr id="post-<?php echo $row->email_id?>" class="post-<?php echo $row->email_id?> hentry alternate iedit author-self level-0">
                        ...
                        <td class="name column-name"><?php  ?></td>
